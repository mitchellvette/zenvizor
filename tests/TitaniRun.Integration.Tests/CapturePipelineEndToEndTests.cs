...
namespace TitaniRun.Integration.Tests;
...
public sealed class CapturePipelineEndToEndTests : IDisposable
{
    ...
    public CapturePipelineEndToEndTests()
    {
        _dbPath = Path.Combine(
            Path.GetTempPath(),
            $"titanirun-e2e-{Guid.NewGuid():N}.db");
    ...
    public void Observe_DoesNotTouchDatabase_OnlyFlushWrites()
    {
        ...
        sizeAfterObserve.Should().Be(sizeBeforeObserve, "Observe() must not write to titanirun.db");
        walAfter.Should().Be(walBefore, "Observe() must not write to titanirun.db-wal");
    }
