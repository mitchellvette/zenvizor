...
- **WPF gotcha (superseded 2026-06-11)**: the earlier guidance here (MaxHeight strategy + asymmetric `Padding="0,5,0,1"`) treated the symptom, not the cause, and the descender clipping kept regressing. Measured root cause: glyph clipping in fixed-height grid cells comes from WPF's *layout clip* — whenever a TextBlock's desired height exceeds its arrange slot (row height minus vertical cell padding), WPF clips to the slot regardless of `ClipToBounds`. Fix: vertical cell padding must be 0 in fixed-height rows (centering comes from the cell template's ContentPresenter), and cell text styles use `LineHeight="Auto"` so the line box is the font's natural one. The cell.body.trim style in `PerAppPage.xaml` and the `style.datagrid.compact` comment in `DesignTokens.xaml` are the reference.
